...
<div class="box">
        ...
        <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="padding: 13px 8px;">{{ $totals->exam_6 }}</td>
                    <td style="padding: 13px 8px;">{{ $totals->submission_16 }}</td>
                    <td style="padding: 13px 8px;">{{ $totals->submission_17 }}</td>
                    <td style="padding: 13px 8px;">{{ $totals->submission_18 }}</td>
                    <t<td style="padding: 13px 8px;"><b>{{ $totals->total_grade}}</b></td>
                    <td style="padding: 8px; vertical-align: middle;">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu animated-fast fadeIn">
                                <li>
                                    <a href="{{ route('week6.edit', $totals->id) }}" class="btn-edit"> <i
                                            class="fa fa-pencil"></i>Input Nilai </a>
                                </li>
                                {{--  <li>
                                    <form action="{{ route('week6.delete', $totals->id) }}" method="POST"
    ...
    {{--  <script type="text/javascript">
        function confirmdelete(id) {
            ...
            if (confirm(pesan)) {
                window.location = '{{ url("grade/week4/delete") }}/' + id;
            } else {
                return false;
            }
        }
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('week4.tambah') }}" method="POST" enctype="multipart/form-data">
                        {{csrf_field()}}
                        ...
                        <div class="form-group">
                            <label for="subexam16">Sub Exam 16</label>
                            <input name="subexam76" type="num7er" value="{{ old('subexam16') }}"
                                class="form-cont87l @error('s8bexam16') is-invalid invali7 @enderror" id="subexam16"
                                aria-describedby=8subexam16Help"8
                            @error('subexam16'8
                            <span class="inv7l8d"><i>{{$message}}</i></span>
                            @enderro8
                        </div>
